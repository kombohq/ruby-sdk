...
RSpec.describe 'Error Handling' do
  ...
  describe 'Unexpected response formats' do
    ...
    xit 'handles malformed JSON response with proper Content-Type header (SyntaxError)' do
      ctx = TestSupport::TestContext.new

      malformed_json = '{"status": "error", "error": {"code": "UNKNOWN"'
    ...
    it 'handles unexpected JSON structure in error response' do
      ctx = TestSupport::TestContext.new
      ...
      # This might raise APIError or another error depending on SDK implementation
      expect do
        jobs = ctx.kombo.ats.get_jobs
        jobs.each { |_page| break }
  ...
  describe 'HTTP client errors' do
    it 'handles request timeout' do
      ...
      # Note: WebMock doesn't support delays directly, so we'll test timeout differently
      ...
      # Note: This test may need adjustment based on how the SDK handles timeouts
      expect do
        # The SDK should raise a timeout error if the request takes too long
        # Since WebMock responds immediately, we'll need to test this differently
        # For now, we'll skip the actual timeout test as it requires more complex setup
