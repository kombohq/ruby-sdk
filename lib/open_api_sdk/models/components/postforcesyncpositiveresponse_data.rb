# Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

# typed: true
# frozen_string_literal: true


module OpenApiSDK
  module Models
    module Components

      class PostForceSyncPositiveResponseData
        extend T::Sig
        include Crystalline::MetadataFields

        # We only allow 1 concurrent sync to be running or queued.
        field :already_queued, Crystalline::Boolean.new, { 'format_json': { 'letter_case': ::OpenApiSDK::Utils.field_name('already_queued'), required: true } }
        # ID of the newly-created or already-queued-or-running sync.
        field :sync_id, ::String, { 'format_json': { 'letter_case': ::OpenApiSDK::Utils.field_name('sync_id'), required: true } }
        # The type of the triggered sync. May differ from request, if the integration does not support delta syncs or if another sync is already running.
        field :type, Models::Components::PostForceSyncPositiveResponseType, { 'format_json': { 'letter_case': ::OpenApiSDK::Utils.field_name('type'), required: true, 'decoder': Utils.enum_from_string(Models::Components::PostForceSyncPositiveResponseType, false) } }

        sig { params(already_queued: T::Boolean, sync_id: ::String, type: Models::Components::PostForceSyncPositiveResponseType).void }
        def initialize(already_queued:, sync_id:, type:)
          @already_queued = already_queued
          @sync_id = sync_id
          @type = type
        end

        sig { params(other: T.untyped).returns(T::Boolean) }
        def ==(other)
          return false unless other.is_a? self.class
          return false unless @already_queued == other.already_queued
          return false unless @sync_id == other.sync_id
          return false unless @type == other.type
          true
        end
      end
    end
  end
end
